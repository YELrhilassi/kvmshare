...
fn hook_messages_decode_transitions() {
    ...
    assert_eq!(from_wparam(wm::WM_LBUTTONDOWN, 0), Some((buttons::LEFT, true)));
    assert_eq!(from_wparam(wm::WM_LBUTTONUP, 0), Some((buttons::LEFT, false)));
    assert_eq!(from_wparam(wm::WM_RBUTTONDOWN, 0), Some((buttons::RIGHT, true)));
    assert_eq!(from_wparam(wm::WM_RBUTTONUP, 0), Some((buttons::RIGHT, false)));
    assert_eq!(from_wparam(wm::WM_MBUTTONDOWN, 0), Some((buttons::MIDDLE, true)));
    assert_eq!(from_wparam(wm::WM_MBUTTONUP, 0), Some((buttons::MIDDLE, false)));

    // Extra buttons: identity in the high word of mouseData.
    let x1 = (wm::XBUTTON1 as u32) << 16;
    let x2 = (wm::XBUTTON2 as u32) << 16;
    assert_eq!(from_wparam(wm::WM_XBUTTONDOWN, x1), Some((buttons::EXTRA_1, true)));
    assert_eq!(from_wparam(wm::WM_XBUTTONUP, x1), Some((buttons::EXTRA_1, false)));
    assert_eq!(from_wparam(wm::WM_XBUTTONDOWN, x2), Some((buttons::EXTRA_2, true)));
    assert_eq!(from_wparam(wm::WM_XBUTTONUP, x2), Some((buttons::EXTRA_2, false)));
