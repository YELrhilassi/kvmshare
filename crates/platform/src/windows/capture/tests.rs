...
fn hook_buttons_map_by_message() {
    ...
    assert_eq!(
        super::buttons::from_wparam(wm::WM_LBUTTONDOWN, 0),
        Some((ids::LEFT, true))
    );
    assert_eq!(
        super::buttons::from_wparam(wm::WM_LBUTTONUP, 0),
        Some((ids::LEFT, false))
    );
    assert_eq!(
        super::buttons::from_wparam(wm::WM_RBUTTONDOWN, 0),
        Some((ids::RIGHT, true))
    );
    assert_eq!(
        super::buttons::from_wparam(wm::WM_RBUTTONUP, 0),
        Some((ids::RIGHT, false))
    );
    assert_eq!(
        super::buttons::from_wparam(wm::WM_MBUTTONDOWN, 0),
        Some((ids::MIDDLE, true))
    );
    assert_eq!(
        super::buttons::from_wparam(wm::WM_MBUTTONUP, 0),
        Some((ids::MIDDLE, false))
    );
    // Extra buttons carry their identity in the high word of mouseData.
    let x1 = (wm::XBUTTON1 as u32) << 16;
    let x2 = (wm::XBUTTON2 as u32) << 16;
    assert_eq!(
        super::buttons::from_wparam(wm::WM_XBUTTONDOWN, x1),
        Some((ids::EXTRA_1, true))
    );
    assert_eq!(
        super::buttons::from_wparam(wm::WM_XBUTTONUP, x1),
        Some((ids::EXTRA_1, false))
    );
    assert_eq!(
        super::buttons::from_wparam(wm::WM_XBUTTONDOWN, x2),
        Some((ids::EXTRA_2, true))
    );
    assert_eq!(
        super::buttons::from_wparam(wm::WM_XBUTTONUP, x2),
        Some((ids::EXTRA_2, false))
    );
