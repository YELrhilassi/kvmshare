...
fn xi2_device_grab_does_not_suppress_raw_events_for_other_clients() {
    ...
    let (Ok((observer, screen_num)), Ok((grabber, _))) =
        (RustConnection::connect(None), RustConnection::connect(None))
    else {
    ...
    assert!(
        baseline > 0,
        "XTEST motion should reach the observer (baseline {baseline})"
    );
    ...
    let devices = xinput::xi_query_device(&grabber, DEVICE_ALL)
        .unwrap()
        .reply()
        .unwrap();
    ...
    assert!(
        after > 0,
        "raw motion must resume after ungrab (got {after})"
    );
}
