...
fn motion_is_accumulated_and_rate_limited() {
    let msgs = collect(&[
        rel(RelativeAxisCode::REL_X.0, 5),
        rel(RelativeAxisCode::REL_Y.0, -3),
    ]);
...
fn buttons_map_to_canonical_ids() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            Message::MouseButton {
                button: buttons::EXTRA_1,
                pressed: true
            },
            Message::MouseButton {
                button: buttons::EXTRA_2,
                pressed: true
            },
        ]
    );
...
fn scroll_lock_is_the_escape_not_a_key() {
    let msgs = collect(&[
        key(KeyCode::KEY_SCROLLLOCK.0, 1),
        key(KeyCode::KEY_SCROLLLOCK.0, 0),
    ]);
