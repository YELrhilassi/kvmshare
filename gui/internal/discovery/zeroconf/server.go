...
func Register(instance, service, domain string, port int, text []string, ifaces []net.Interface) (*Server, error) {
	...
	s.service = entry
	// VENDORED-FIX: the receiver goroutines' WaitGroup counters are
	// Added **here, synchronously before Register returns** — upstream
	// Added them inside the goroutines, so a fast Shutdown() raced the
	// Adds. Synchronous Add + happens-before goroutine spawn means
	// Shutdown's Wait() always has exact knowledge of the receivers,
	// whichever goroutine hierarchy spawns them.
	if s.ipv4conn != nil {
		s.shutdownEnd.Add(1)
	}
	if s.ipv6conn != nil {
...
func RegisterProxy(instance, service, domain string, port int, host string, ips []string, text []string, ifaces []net.Interface) (*Server, error) {
	...
	s.service = entry
	// VENDORED-FIX: same synchronous Add discipline as Register.
	if s.ipv4conn != nil {
		s.shutdownEnd.Add(1)
	}
	if s.ipv6conn != nil {
...
func (s *Server) mainloop() {
	// The shutdownEnd counters were Added by the caller (VENDORED-FIX):
	// spawning them here would reintroduce the Add/Wait race one level
	// up, because this function is itself a goroutine.
	if s.ipv4conn != nil {
...
func (s *Server) recv4(c *ipv4.PacketConn) {
	...
	buf := make([]byte, 65536)
	// Add(1) for this receiver happens in mainloop, before the spawn
	// (VENDORED-FIX) — Done here pairs with it.
